...
int main()
{
    struct slab * my_slab = mmap(NULL, sizeof(struct slab)+sizeof(struct seqlock), PROT_READ|PROT_WRITE, MAP_SHARED|MAP_ANONYMOUS, -1, 0);
    if(my_slab == (void*)-1)
    {
    ...
    for(int i = 0; i < 8; i++) //create 4 children
    {
        ...
        else if(fork_result == 0)
        {
            for (int j=0; j < 100000; ++j) //Each child will do 100,000 random operations
            {
                ...
                {
                    case 0:
                        result = dll_insert_seq(anchor, value, my_slab);
                        ...
                        break;
                    case 1:
                        result = dll_find_seq(anchor, value);
                        //fprintf(stderr, "Find\n");
                        break;
                    case 2:
                        ...
                        //    fprintf(stderr, "Deletion value %d is founded.\n", value);   
                            dll_delete_seq(anchor, result, my_slab);
